...
int main(int argc, char **argv)
{
	...
	vals_alt = (uint64_t*)malloc(nvals*sizeof(vals_alt[0]));
	RAND_bytes((unsigned char *)vals_alt, sizeof(*vals_alt) * nvals);
	for (uint64_t i = 0; i < nvals; i++) {
	   vals_alt[i] = (1 * vals_alt[i]) %qf.metadata->range;
	}
